      ...
      data.name.toLowerCase() === 'launch vehicle' ? 'tech-tab--active' : ''
        ...
        <a class="tech-tab ${
          data.name.toLowerCase() === 'spaceport' ? 'tech-tab--active' : ''
            ...
            <a class="tech-tab ${
              data.name.toLowerCase() === 'space capsule' ? 'tech-tab--active' : ''
...
const getTechData = function (techName) {
  fetch('data.json')
    ...
    .then((data) => {
      const techData = data.technology.find((tech) => tech.name.toLowerCase() === techName)
      if (techData) {
        renderTech(techData)
      } else {
...
window.addEventListener('resize', () => {
  if (techContainer) {
    const currentTech = document.querySelector('.tech-title').textContent.toLowerCase()
