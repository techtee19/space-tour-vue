...
const renderCrew = function (data, className = '') {
  const html = ` <div class="crew-cont ${className}">
          <div class="crew-text-cont">
            ...
            <div class="dots">
              <a class="dots-dot ${
                data.role.toLowerCase() === 'commander' ? 'dots__dot--active' : ''
              }" data-crew="Commander"></a>
              <a class="dots-dot ${
                data.role.toLowerCase() === 'mission specialist' ? 'dots__dot--active' : ''
              }" data-crew="Mission Specialist"></a>
              <a class="dots-dot ${
                data.role.toLowerCase() === 'pilot' ? 'dots__dot--active' : ''
              }" data-crew="Pilot"></a>
              <a class="dots-dot ${
                data.role.toLowerCase() === 'flight engineer' ? 'dots__dot--active' : ''
...
const getCrewData = function (crewRole) {
  fetch('data.json')
    ...
    .then((data) => {
      const crewMember = data.crew.find((crew) => crew.role.toLowerCase() === crewRole)
      if (crewMember) {
